...
(function($) {
    $.fn.makeAsyncUploader = function(options) {
        return this.each(function() {
            ...
            container.append($("<span id='" + id + "_uploading'>Cargando... <input type='button' value='Cancelar'/></span>"));
            container.append($("<span id='" + id + "_swf'/>"));
            container.append($("<input type='hidden' name='" + id + "_filename' id='" + id + "_filename' />"));
            container.append($("<input type='hidden' name='" + id + "_guid' id='" + id + "_guid' />"));
            ...
            var defaults = {
                ...
                button_text: "<font face='Arial' size='13pt'>Seleccione Archivo</span>",
                ...
                file_queue_error_handler: function(file, code, msg) { alert("Perdon, Tu archivo no pudo ser subido: " + msg); },

                // Called when an error occurs during upload
                upload_error_handler: function(file, code, msg) { alert("Perdon, Tu archivo no pudo ser subido: " + msg); },
                ...
                upload_success_handler: function(file, response) {
					...
					$("span[id$=_completedMessage]", container).html("cargado <b><a href='" + options.path_url + response + "' rel='shadowbox;" + (file.name.indexOf(".flv") >= 0 ? "width=480;height=240;" : "") + "'>" + file.name + "</a></b> (" + Math.round(file.size / 1024) + " KB) ");
					Shadowbox.setup();
                },
            ...
            if (options.existingFilename || "" != ""){
				$("span[id$=_completedMessage]", container).html("Cargado <b><a href='" + options.path_url + options.existingGuid + "' rel='shadowbox;" + (options.existingFilename.indexOf(".flv") >= 0 ? "width=480;height=240;" : "") + "'>" + options.existingFilename + "</a></b> (" + (options.existingFileSize ? Math.round(options.existingFileSize / 1024) : "?") + " KB) ");
