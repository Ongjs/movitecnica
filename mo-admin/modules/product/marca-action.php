...
switch ($do) {
    case 5:
        ...
        $cn->query("UPDATE product SET name = '$name', parent_id = '$parent', image = '$image', content = '$content' WHERE id = '" . mo_scape($_POST['id']) . "'");
        break;
    case 2:
        $name = mo_scape($_POST['name']);
        $image = mo_scape($_POST["image_guid"]);
        $content = mo_scape($_POST['cont']);
        $parent = mo_scape($_POST['opt_cat']);
        $cn->query("insert into product(id,parent_id,type,name,image,content) VALUES ('',$parent,'2','$name','$image','$content')");
        break;
    case 4:
