...
switch ($do) {
    case 5:
        ...
        $cn->query("UPDATE product SET name = '$name', image = '$image', content = '$content' WHERE id = '" . $_POST['id'] . "'");
        break;
    case 2:
        $name = mo_scape($_POST['name']);
        $image = mo_scape($_POST["image_guid"]);
        ...
        break;
    case 4:
        $cn->query("DELETE from product WHERE id = " . $_POST['id']);
        break;
    case 3: 
	$cn->query("UPDATE product SET status = '".($_POST['value'] == 1 ? 0 : 1)."' WHERE id = '".$_POST['id']."'");
	break;
}