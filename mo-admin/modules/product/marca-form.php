...
if (!empty($id)) {
    $cn->query("SELECT name, parent_id, image, content from product WHERE id = '$id'");
    $row = $cn->fetch();
        ...
        <table width="100%">
            ...
            <tr>
                ...
                <td>
                    ...
                    if (!$cn->numrows())
                        exit;
                    echo '<select name="opt_cat">';
                    while ($row2 = $cn->fetch()) {
                        $selected = $ext == $row2['id'] ? "selected" : "";
                        echo '<option value="' . $row2['parent_id'] . '" ' . $selected . '>' . $row2['name'] . '</option>';
                    }
                    echo '</select>';
                    ?>
                </td>
