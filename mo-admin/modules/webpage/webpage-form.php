...
if (!empty($id)) {
    $cn->query("SELECT name, image, content, description, category_id, thumbnail from content WHERE id = '$id'");
    $row = $cn->fetch();
}
?>
<form id="<?php echo empty($id) ? "save" : "update"; ?>">
    <fieldset>
        ...
        <table width="100%">
            ...
            ?>
            <tr>
                <td><label for="image">miniatura<br /><span class="recommended_size">(160w x 80h)</span></label></td>
